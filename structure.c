    ...
    while (1) {
        ...
        for (int i = 0; i < data_size; i++) {
            if (i == 26 || i == 27 || i == 28 || i == 29) {
                packet.srcIP[(i-26)] = buffer[i];
            }
        }
        printf("%d.%d.%d.%d", packet.srcIP[0], packet.srcIP[1], packet.srcIP[2], packet.srcIP[3]);
        printf("\nDestination IP: ");
        for (int i = 0; i < data_size; i++) {
            if (i == 30 || i == 31 || i == 32 || i == 33) {
                packet.dstIP[(i-30)] = buffer[i];
            }
        }
        printf("%d.%d.%d.%d", packet.dstIP[0], packet.dstIP[1], packet.dstIP[2], packet.dstIP[3]);
        printf("\n");
    }
    close(rawSocket);
}